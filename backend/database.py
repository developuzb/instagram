...
async def create_account(instagram_id, username, page_access_token=None,
                         page_id=None, ig_username=None, ig_session=None, auth_type="token"):
    db = await get_db()
    try:
        await db.execute(
...
async def update_account_session(account_id, ig_session):
    db = await get_db()
    try:
...
async def get_triggers(account_id=None):
    db = await get_db()
    try:
        if account_id:
...
async def create_trigger(account_id, keyword, reply_message, match_all=0, trigger_type='comment'):
    db = await get_db()
    try:
        cursor = await db.execute(
...
async def update_trigger(trigger_id, keyword, reply_message, is_active, match_all=0, trigger_type='comment'):
    db = await get_db()
    try:
        await db.execute(
...
async def find_matching_trigger(account_id, comment_text, trigger_type='comment'):
    db = await get_db()
    try:
...
async def get_posts(account_id=None):
    db = await get_db()
    try:
        if account_id:
            cursor = await db.execute(
...
async def add_post(account_id, post_id, post_url="", caption="", media_numeric_id=None):
    db = await get_db()
    try:
        await db.execute(
...
async def update_post_media_id(post_shortcode, media_numeric_id):
    db = await get_db()
    try:
        await db.execute(
...
async def toggle_post_monitoring(post_id, is_monitoring):
    db = await get_db()
    try:
...
async def get_subscribers(account_id=None):
    db = await get_db()
    try:
        if account_id:
            cursor = await db.execute(
...
async def upsert_subscriber(account_id, instagram_user_id, username=""):
...
async def get_messages_log(account_id=None, limit=100):
    db = await get_db()
    try:
        if account_id:
            cursor = await db.execute("""
...
async def get_stats():
    ...
    try:
        ...
        for table, label in [
            ...
            stats[label] = (await cursor.fetchone())[0]
        cursor = await db.execute(
